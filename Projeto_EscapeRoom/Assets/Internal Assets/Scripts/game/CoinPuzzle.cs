...
public class CoinPuzzle : MonoBehaviour
{
    ...
    // Função para colocar as moedas nos slots selecionados na ordem fixa
    ...
    private void CheckSolution()
    {
        ...
        if (isCorrect)
        {
            ...
            // Aqui você pode adicionar efeitos: abrir porta, ativar animação, etc.
            // Exemplo: GetComponent<Animator>().SetTrigger("Solved");
        }
        else
        {
            Debug.Log("Combinação incorreta. A ordem das moedas não matches a solução. Tente novamente.");
            // Opcional: Remova as moedas erradas ou dê feedback visual (já reseta no PlaceCoinsInSelectedSlots se quiser)
        }
    }
